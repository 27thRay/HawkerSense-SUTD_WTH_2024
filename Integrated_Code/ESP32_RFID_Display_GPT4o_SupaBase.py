    ...
    try:
        ...
        async for message in websocket:
            ...
            if  message == "stall 1":
                ...
                time.sleep(2)
                text_response = load_saved_image_to_GPT()
...
def upload_To_SupaBase(stall, rice, meat, veggies):
    ...
    if response.data:  # Assuming 'data' contains the inserted record(s) on success
...
def load_saved_image_to_GPT():
    # Load saved image from the folder
    IMAGE_FOLDER = "captured_images"
    ...
    # Convert the latest image to base64
...
def extract_portions_from_text(text):
    ...
    if start != -1 and end != -1:
        ...
        # Split the string by commas, strip any extra spaces, and convert to floats
        portions = portions_string.split(',')
        if len(portions) == 3:  # Ensure there are exactly three values
            rice, veggies, meat = [float(p.strip()) for p in portions]
